...
option_list = list(
  ...
  make_option(c("-m", "--methylation_file"), type="character", default=NULL,
              help="path to methylation file, modified_bases.5mC.bed", metavar="character"),
  ...
  make_option(c("-d", "--training_data"), type="character", default="capper_top_100k_betas_binarised.Rdata",
              help="Top 100k from Capper dataset", metavar="character"),
...
filein <- opt$methylation_file
meth <- read.delim(filein,header=FALSE)
